...
import HotelCard from "../Food/FoodCard";
import Pagination from "../Common/Paganation";
import AddHotel from "../Food/AddEdit";
...
const HotelPage = () => {
  ...
  const confirmDelete = () => {
    dispatch(deleteFood(deleteId, () => loadHotels(currentPage, searchTerm, filters)));
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      <div className="max-w-7xl mx-auto p-4 md:p-6">
        ...
        {/* ERROR MESSAGE */}
        {error && (
          <div className="mt-6 p-4 bg-red-800 text-red-200 rounded-lg text-center">
            ⚠️ {error || "Something went wrong while fetching hotels."}
          </div>
        )}
        ...
        {!loading && !error && hotels.length === 0 && (
          <div className="mt-10 text-center text-gray-400 text-lg">
            No hotels found. Try adjusting filters or search.
          </div>
        )}

        {/* LOADING */}
        {loading && (
          <div className="mt-10 text-center text-yellow-400 animate-pulse">
            Loading hotels...
          </div>
        )}
      ...
      {showAddModal && (
        <AddHotel
          ...
          onSuccess={() => loadHotels(currentPage, searchTerm, filters)}
        />
      )}
