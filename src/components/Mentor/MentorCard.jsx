...
import React from 'react';
import { GraduationCap, Phone, FileText } from 'lucide-react';
import { toast } from 'react-hot-toast';
...
const MentorCard = ({ mentor }) => {
  ...
  return (
    <div className="bg-[#1A1B1F] border border-gray-400 rounded-xl shadow-md overflow-hidden hover:shadow-yellow-400/20 transition-all duration-300 mb-6 flex flex-col h-full">
      <div className="relative w-full h-[220px] overflow-hidden rounded-t-xl bg-black">
        <img
      ...
      <div className="p-4 space-y-2 bg-gray-900 flex-grow">
        <h3 className="text-center text-white font-bold text-lg">{mentor.name}</h3>
        ...
        <div className="flex flex-col items-center justify-center text-sm text-gray-300 gap-1 min-h-[90px]">
          <span className="text-yellow-400">Companies:</span>
          <ul className="list-disc text-center">
            {mentor.companies.map((company, index) => (
              <li key={index}>{company}</li>
